...
<img src="무하유_유해콘텐츠_데이터_모델선정/최종_모델/project_banner.jpg" width="500" alt="Project Banner">
...
│   └── 최종_모델/                   # 최종 모델 평가 스크립트
│       ├── 박상원/
│       │   ├── harmful_content_demo/    # 웹 데모 버전
│       │   └── harmful_content_export/   # 평가용 버전
│       ├── 안지산/
│       └── 임영재/
│
├── 박상원/                           # 팀원 1 작업 폴더
│   ├── 모델_발전_과정/               # 모델 발전 과정
│   │   ├── final_model1/ ~ final_model11/  # 모델 버전별 코드
│   │   └── 파일럿_테스트/            # 초기 파일럿 테스트
│   ├── 팀원_라벨링/                 # 초기 라벨링 도구
...
프로젝트 루트에 통합된 `.gitignore` 파일이 있으며, 다음 항목들이 Git에서 제외됩니다:

- **데이터 파일**: 이미지(*.jpg, *.png 등), 비디오(*.mp4, *.avi 등), 오디오 파일
- **모델 가중치**: *.pth, *.pt, *.h5, *.ckpt 등
- **가상환경**: venv/, env/, ENV/ 등
- **Python 캐시**: __pycache__/, *.pyc 등
- **IDE 설정**: .vscode/, .idea/ 등
- **로그 및 임시 파일**: *.log, *.tmp, *실행결과*.txt 등

> 폴더 구조는 유지되며, 파일만 제외됩니다. 빈 폴더를 유지하기 위해 `.gitkeep` 파일이 일부 폴더에 포함되어 있습니다.

    ...
    try:
        ...
        for i in range(clip_count):
            ...
            for window_start, window_end in windows:
                ...
                # 결과 집계
                if window_result.get("is_harmful", False):
                    any_harmful = True
        ...
        # 최종 결과 반환 (영재님 방식: 모든 윈도우 중 최대 confidence 사용)
        return {
            "is_harmful": any_harmful or (max_confidence > threshold),
            "confidence": max_confidence,
            "detected_objects": list(all_detected_objects),
            "detected_behaviors": list(all_detected_behaviors),
            "num_windows_processed": len(window_results),

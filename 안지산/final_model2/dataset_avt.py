...
class AVTDataset(Dataset):
    """
    manifest(jsonl) 각 라인 예시 (split_manifest.py 기준):
    ...
    def __init__(self, manifest_path: str, cfg: dict, tokenizer):
        super().__init__()
    ...
    def _load_video(self, path: str) -> torch.Tensor:
        ...
        if (decord is None) or (path is None) or (not os.path.exists(path)):
        ...
        frames = vr.get_batch(idxs)  # (T,H,W,C) torch.Tensor
    ...
    # Text loader
    # -------------------------------
    def _load_text(self, path: str) -> Tuple[torch.Tensor, torch.Tensor]:
        text = ""
    ...
    def __getitem__(self, idx: int):
        ...
        vid = self._load_video(vid_path)      # (T,C,H,W)
        aud = self._load_audio(aud_path)      # (1,N)
        ids, mask = self._load_text(txt_path) # (L,), (L,)

        # label 필드 이름: 우선 harmful, 없으면 label fallback
        if "harmful" in it:
            y = float(it["harmful"])
        else:
            y = float(it.get("label", 0))

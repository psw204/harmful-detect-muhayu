"""
무하유 유해 콘텐츠 탐지 시스템 - 최적화 및 안정화 버전 10
이미지: YOLOv8 + CLIP + 행동 인식 + 차원 축소 MLP
비디오: YOLOv8 + SlowFast + CLIP + 행동 인식 + 경량 Transformer

이 시스템은 이미지와 비디오에서 유해한 콘텐츠와 행동을 자동으로 탐지하는 AI 모델입니다.
- 이미지: YOLO + CLIP + 행동 + 차원 축소 레이어 + MLP
- 비디오: YOLO + SlowFast + CLIP + 행동 + 경량 Transformer

[변경점] final_model10: Model 9 문제점 해결
- 차원 축소 레이어 추가 (940 → 256): 과적합 방지 및 학습 효율 향상
- Focal Loss 적용: 어려운 샘플에 집중하여 성능 개선
- CLIP 특징 정규화 (F.normalize): 안정적 학습 보장
- 재현성 100% 보장 (seed 고정): 실험 결과 재현 가능
- Early Stopping: validation loss 기준으로 변경하여 과최적화 방지
- 프레임 에러 처리 강화: 손상된 비디오 프레임 자동 스킵
- 행동 인식 경량화 (프롬프트 1개): 학습 시간 단축
...
# ------------------------------------------------------------
# [변경점] final_model10: 행동 인식 경량화
# ------------------------------------------------------------
# 프롬프트를 1개로 축소하여 학습 속도 향상 (Model 9: 3개 → Model 10: 1개)
...
ALL_OBJECTS = HARMFUL_OBJECTS + CONTEXTUAL_OBJECTS  # 총 21종 (12 + 9)

# ============================================================
# [변경점] final_model10: Focal Loss 구현
# ============================================================
        ...
        for behavior, prompt in BEHAVIOR_PROMPTS.items():
            ...
            for frame in frames[:min(len(frames), 4)]:
                ...
                with torch.no_grad():
                    ...
                    # ------------------------------------------------------------
                    # [변경점] final_model10: CLIP 특징 정규화
                    # L2 정규화로 코사인 유사도 계산 안정화
                    # ------------------------------------------------------------
...
class HarmfulImageDataset(Dataset):
    """
    유해 이미지 탐지 데이터셋 (경량화 + 행동 인식)
    
    각 이미지에서 다음을 추출:
    1. YOLO 특징 (21차원): 객체 탐지 결과
    2. CLIP 특징 (512차원): 이미지 맥락 이해
    3. 행동 특징 (7차원): Zero-shot + 규칙 기반 행동 감지
    
    총 입력 차원: 21 + 512 + 7 = 540차원 → 차원 축소 → 256차원
    """
    ...
    def _extract_yolo_features(self, results):
        ...
        Returns:
            feature_vector: 객체별 탐지 개수 벡터 (21차원)
...
class HarmfulVideoDataset(Dataset):
    ...
    총 입력 차원: 21 + 512 + 400 + 7 = 940차원/프레임
    시퀀스 차원: (32, 940) → 차원 축소 → (32, 256)
    """
    ...
    def __getitem__(self, idx):
        ...
        try:
            # ------------------------------------------------------------
            # [변경점] final_model10: 프레임 에러 처리 강화
            # ------------------------------------------------------------
            # 손상된 프레임을 자동으로 스킵하는 안전한 프레임 추출
            ...
            yolo_features_seq = torch.stack(yolo_features_list)  # (32, 21)
            ...
            # ------------------------------------------------------------
            # 5. 전체 특징 결합 (32, 21+512+400+7 = 32, 940차원)
            # ------------------------------------------------------------
            combined = torch.cat([
                yolo_features_seq,
                clip_features_seq,
...
# ============================================================
# [변경점] final_model10: 차원 축소 레이어가 포함된 이미지 모델
# ============================================================
class HarmfulImageClassifier(nn.Module):
    ...
    def __init__(self, yolo_dim, clip_dim, behavior_dim):
        ...
        # 540 → 256 차원 축소로 과적합 방지 및 학습 효율 향상
        # ------------------------------------------------------------
...
# ============================================================
# [변경점] final_model10: 차원 축소 + 경량 Transformer 비디오 모델
# ============================================================
class HarmfulVideoClassifier(nn.Module):
    """
    유해 비디오 분류 모델 (차원 축소 + 경량 Transformer)
    ...
    차원 축소로 얻는 이점:
    - Transformer nhead: 1 → 8 (Model 9 대비 8배 향상)
    - 학습 시간: 3시간 50분 → 2시간 30분 예상 (35% 단축)
    """
    def __init__(self, yolo_dim, clip_dim, slowfast_dim, behavior_dim):
        ...
        # ------------------------------------------------------------
        # [변경점] final_model10: 차원 축소 레이어 추가
        # 940 → 256 차원 축소로 Transformer 효율 향상 (nhead 8 가능)
        # ------------------------------------------------------------
        reduced_dim = 256
        self.dimension_reduction = nn.Sequential(
        ...
        # Transformer Encoder
        # nhead=8: attention head 개수 (256은 8로 나누어떨어짐)
        # Model 9에서는 nhead=1이었지만, 차원 축소로 nhead=8 가능!
        nhead = 8
    ...
    def forward(self, x):
        ...
        x = x.view(-1, feat_dim)  # (batch*32, 940)
...
def prepare_image_data():
    ...
    3. 수집 데이터: 직접 수집한 유해 이미지
    4. 안전 데이터: 직접 수집한 안전 이미지
...
def prepare_video_data():
    ...
    vpaths, vlabels = [], []   # 유해 비디오
    svpaths, svlabels = [], [] # 안전 비디오
...
# ============================================================
# [변경점] final_model10: Early Stopping - Validation Loss 기준
# ============================================================
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, model_type='image'):
    ...
    [변경점] Model 10: F1이 아닌 Val Loss 기준으로 Early Stopping
    → Threshold 과최적화 방지 및 일반화 능력 향상
    ...
    for epoch in range(epochs):
        ...
        # ========================================
        # [변경점] final_model10: Val Loss 기준 Early Stopping
        # ========================================
...
if __name__ == '__main__':
    print("="*60)
    print("무하유 유해 콘텐츠 탐지 시스템 - 최적화 버전 10")
    print("="*60)
    print(f"Device: {DEVICE}")
    print(f"Harmful Objects: {len(ALL_OBJECTS)}개")
    print(f"Harmful Behaviors: {len(HARMFUL_BEHAVIORS)}개 (경량화)")

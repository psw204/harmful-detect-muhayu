...
- 차원 축소 레이어: 고차원 특징(537/937차원)을 256차원으로 압축하여 과적합 방지
...
# 비디오 처리 설정
FRAME_SAMPLE = 32       # 비디오에서 샘플링할 프레임 수 (시간적 해상도, SlowFast 모델 호환성을 위해 32로 설정)
...
class HarmfulImageDataset(Dataset):
    """
    유해 이미지 탐지 데이터셋 (카테고리 기반 행동 인식 포함)
    
    각 이미지에서 다음을 추출:
    1. YOLO 특징 (17차원): 객체 탐지 결과 (카테고리별 매핑)
    2. CLIP 특징 (512차원): 이미지 맥락 이해
    3. 행동 특징 (8차원): 카테고리별 Zero-shot + 규칙 기반 감지
    
    총 입력 차원: 17 + 512 + 8 = 537차원 → 차원 축소 → 256차원
    """
    def __init__(self, image_paths, labels, yolo_model, clip_model, clip_preprocess, augment=False):
        ...
        # 텍스트 특징 캐싱 (카테고리별 여러 프롬프트 지원)
        # 각 카테고리의 모든 프롬프트를 인코딩하여 재사용
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # ------------------------------------------------------------
            # 3. 행동 특징 추출 (8차원) - 카테고리 기반, Release2 스타일 최적화
            # ------------------------------------------------------------
            # 이미 추출한 CLIP 특징을 재사용하여 행동 감지 (속도 향상)
            behavior_scores = detect_behavior_with_clip_fast_optimized(
                clip_features, self.clip_model, self.text_features_cache
            )
            
            # 객체 기반 카테고리 추론
            inferred_categories = infer_behavior_from_objects(object_counts)
            
            # 두 방법의 결과를 결합 (CLIP 60% + 규칙 40%, Precision 향상을 위해 규칙 비중 상향)
            behavior_features = torch.zeros(len(BEHAVIOR_CATEGORIES))
            ...
            # ------------------------------------------------------------
            # 4. 전체 특징 결합 (17 + 512 + 8 = 537차원)
            # ------------------------------------------------------------
            combined = torch.cat([yolo_features, clip_features_cpu, behavior_features])
            ...
            # 에러 시 zero 벡터 반환
            zero_dim = len(ALL_OBJECTS) + 512 + len(BEHAVIOR_CATEGORIES)
    ...
    def _extract_yolo_features(self, results):
        ...
        Returns:
            feature_vector: 객체별 탐지 개수 벡터 (17차원)
...
class HarmfulVideoDataset(Dataset):
    """
    유해 비디오 탐지 데이터셋 (카테고리 기반 행동 인식 포함)
    
    각 비디오에서 32개 프레임을 샘플링하고, 프레임별로 다음을 추출:
    1. YOLO 특징 (17차원): 객체 탐지 결과 (카테고리별 매핑)
    2. CLIP 특징 (512차원): 프레임 맥락 이해
    3. SlowFast 특징 (400차원): 시간적 행동 패턴
    4. 행동 특징 (8차원): 카테고리별 Zero-shot + 규칙 기반 감지
    
    총 입력 차원: 17 + 512 + 400 + 8 = 937차원/프레임
    시퀀스 차원: (32, 937) → 차원 축소 → (32, 256)
    """
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            yolo_features_seq = torch.stack(yolo_features_list)  # (32, 17)
            ...
            # 두 방법의 결과를 결합 (CLIP 70% + 규칙 30%, Precision 향상을 위해 규칙 비중 상향)
            behavior_features = torch.zeros(len(BEHAVIOR_CATEGORIES))
            for i, category in enumerate(BEHAVIOR_CATEGORIES):
                clip_score = behavior_scores.get(category, 0.0)
                rule_score = 1.0 if category in inferred_categories else 0.0
            ...
            behavior_features_seq = behavior_features.unsqueeze(0).expand(FRAME_SAMPLE, -1)  # (32, 8)
            
            # ------------------------------------------------------------
            # 5. 전체 특징 결합 (32, 17+512+400+8 = 32, 937차원)
            # ------------------------------------------------------------
            combined = torch.cat([
                yolo_features_seq,
                clip_features_seq,
            ...
            # 에러 시 zero 텐서 반환
            zero_dim = len(ALL_OBJECTS) + 512 + self.slowfast_dim + len(BEHAVIOR_CATEGORIES)
    ...
    def _extract_frames_safe(self, video_path):
        ...
        비디오에서 FRAME_SAMPLE개 프레임을 균등하게 샘플링하되,
        ...
        Returns:
            frames_pil: PIL Image 리스트 (FRAME_SAMPLE개)
            frame_tensors: 프레임 텐서 리스트 (FRAME_SAMPLE개)
        """
        cap = cv2.VideoCapture(video_path)
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        ...
        # 프레임 부족 시 반복하여 FRAME_SAMPLE개 채우기
    ...
    def _extract_slowfast_features(self, frame_tensors):
        """
        SlowFast 모델로 비디오 행동 특징 추출 (동적 프레임 대응 버전)
        
        SlowFast는 두 개의 pathway를 사용:
        - Slow pathway: 8 프레임, 저주파 정보 (의미론적 이해)
        - Fast pathway: 전체 프레임, 고주파 정보 (빠른 움직임)
        
        Args:
            frame_tensors: 프레임 텐서 리스트 (FRAME_SAMPLE 개)
        ...
        try:
            ...
            # 실제 프레임 개수 확인 (FRAME_SAMPLE)
            N = len(frame_tensors_normalized)
            
            # Fast pathway: 전체 프레임 사용
            ...
            # Slow pathway: 8 프레임 균등 샘플링 (0 ~ N-1 범위에서 추출)
            # 기존 코드의 하드코딩된 31을 N-1로 변경하여 IndexError 방지
            slow_indices = torch.linspace(0, N - 1, 8).long()
...
class HarmfulImageClassifier(nn.Module):
    ...
    아키텍처:
    입력 (537차원) → 차원 축소 (256차원) → MLP → 출력 (1차원, 유해 확률)
    ...
    def __init__(self, yolo_dim, clip_dim, behavior_dim):
        ...
        """
        super().__init__()
        input_dim = yolo_dim + clip_dim + behavior_dim  # 537
...
class HarmfulVideoClassifier(nn.Module):
    ...
    아키텍처:
    입력 (32, 937차원) → 차원 축소 (32, 256차원) → Transformer → 시간 평균 풀링 → MLP → 출력 (1차원)
    ...
    def __init__(self, yolo_dim, clip_dim, slowfast_dim, behavior_dim):
        ...
        """
        super().__init__()
        input_dim = yolo_dim + clip_dim + slowfast_dim + behavior_dim  # 937
    ...
    def forward(self, x):
        ...
        x = x.view(-1, feat_dim)  # (batch*32, 937)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, model_type='image'):
    ...
    for epoch in range(epochs):
        ...
        # 여러 threshold에서 F1-Score 계산 (Precision 향상을 위해 높은 threshold 추가)
        for th in [0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]:
...
if __name__ == '__main__':
    ...
    print(f"Behavior Categories: {len(BEHAVIOR_CATEGORIES)}개")
    print(f"  카테고리: {', '.join(BEHAVIOR_CATEGORIES)}")
    ...
    clip_dim = 512
    behavior_dim = len(BEHAVIOR_CATEGORIES)
    ...
    print(f"\n✓ 비디오 클래스 분포: 유해 {num_harmful_v}개, 안전 {num_safe_v}개")
    ...
    # Focal Loss (이미지와 동일하게 alpha=0.25 적용하여 FP 방지)
    criterion_video = FocalLoss(alpha=0.25, gamma=2.0)
    print(f"✓ 손실 함수: Focal Loss (alpha=0.25, gamma=2.0)")

...
이미지: YOLOv8 + CLIP + MLP (앙상블)
비디오: YOLOv8 + SlowFast + CLIP + Transformer (앙상블)

이 시스템은 이미지와 비디오에서 유해한 콘텐츠를 자동으로 탐지하는 AI 모델입니다.
- 이미지: YOLO로 객체 탐지 + CLIP으로 맥락 이해 + 앙상블 MLP로 분류
- 비디오: YOLO + SlowFast로 행동 인식 + CLIP + 앙상블 Transformer로 시계열 분석

[변경점] final_model8: 모델 앙상블 및 Cross Validation을 통한 실전 성능 향상
...
# ------------------------------------------------------------
# [변경점] final_model8: 데이터 중복 제거를 위한 hashlib 추가
# ------------------------------------------------------------
import hashlib
...
# ------------------------------------------------------------
# [변경점] final_model8: Cross Validation 설정
# ------------------------------------------------------------
K_FOLDS = 5             # K-Fold Cross Validation 폴드 수

# ------------------------------------------------------------
# [변경점] final_model8: 앙상블 모델 수 설정
# ------------------------------------------------------------
ENSEMBLE_MODELS = 3     # 앙상블할 모델 개수
...
# ============================================================
# [변경점] final_model8: 이미지 중복 제거 함수 (final_model6과 동일)
# ============================================================
...
# ============================================================
# [변경점] final_model8: 안전한 데이터 증강 추가
# ============================================================
...
class HarmfulImageDataset(Dataset):
    ...
    안전한 데이터 증강을 통해 학습 데이터의 다양성을 증가시킴
    """
...
# ============================================================
# [변경점] final_model8: 앙상블 이미지 분류 모델
# ============================================================
class EnsembleImageHarmfulClassifier(nn.Module):
    """
    앙상블 이미지 유해 콘텐츠 분류 모델
    
    여러 개의 서로 다른 MLP 모델을 앙상블하여 더 안정적인 예측을 수행
    """
    def __init__(self, yolo_dim, clip_dim, num_models=3):
        super().__init__()
        input_dim = yolo_dim + clip_dim  # YOLO + CLIP 특징 차원
        self.num_models = num_models
        
        # ------------------------------------------------------------
        # [변경점] final_model8: 여러 개의 서로 다른 MLP 모델 생성
        # ------------------------------------------------------------
...
# ============================================================
# [변경점] final_model8: 앙상블 비디오 분류 모델
# ============================================================
class EnsembleVideoHarmfulClassifier(nn.Module):
    """
    앙상블 비디오 유해 콘텐츠 분류 모델
    
    여러 개의 서로 다른 Transformer 모델을 앙상블하여 더 안정적인 예측을 수행
    """
    def __init__(self, yolo_dim, clip_dim, slowfast_dim, num_models=3):
        super().__init__()
        input_dim = yolo_dim + clip_dim + slowfast_dim  # 모든 특징 차원 합
        self.num_models = num_models
        
        # ------------------------------------------------------------
        # [변경점] final_model8: 여러 개의 서로 다른 Transformer 모델 생성
        # ------------------------------------------------------------
        self.models = nn.ModuleList()
        for i in range(num_models):
            # 각 모델마다 다른 구조
            if i == 0:
                ...
                # 모델 3: 더 넓은 Transformer
                nhead = max(2, input_dim // 256)
                while input_dim % nhead != 0 and nhead > 1:
                    nhead -= 1
            ...
            self.models.append(nn.ModuleDict({
    ...
    def forward(self, x):
        ...
        for model_dict in self.models:
            ...
            transformed = model_dict['transformer'](x)
            
            # 시퀀스 차원에서 평균 풀링 (전체 비디오 특징)
            pooled = transformed.mean(dim=1)
            
            # 분류기로 최종 예측
            pred = model_dict['classifier'](pooled)
            predictions.append(pred)
...
# ============================================================
# [변경점] final_model8: 강화된 Label Smoothing Loss
# ============================================================
...
# ============================================================
# [변경점] final_model8: Cross Validation 이미지 모델 학습
# ============================================================
def train_image_model_cv():
    """
    Cross Validation을 사용한 이미지 모델 앙상블 학습
    
    K-Fold Cross Validation을 통해 더 엄격한 평가와 안정적인 모델을 생성
    """
    print("\n" + "="*60)
    print("Cross Validation 이미지 모델 앙상블 학습")
    print("="*60)
    ...
    all_train_data = list(zip(train_images, train_labels))
    all_train_data = np.array(all_train_data, dtype=object)
    
    fold_scores = []
    ensemble_models = []
    
    print(f"\n✓ {K_FOLDS}-Fold Cross Validation 시작")
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(all_train_data)):
        ...
        # 폴드별 데이터 분할
        fold_train_data = all_train_data[train_idx]
        fold_val_data = all_train_data[val_idx]
        
        fold_train_images = [item[0] for item in fold_train_data]
        fold_train_labels = [item[1] for item in fold_train_data]
        fold_val_images = [item[0] for item in fold_val_data]
        fold_val_labels = [item[1] for item in fold_val_data]
        
        # 데이터셋 및 데이터로더 생성
        train_dataset = HarmfulImageDataset(fold_train_images, fold_train_labels, yolo, clip_model, clip_preprocess, augment=True)
        val_dataset = HarmfulImageDataset(fold_val_images, fold_val_labels, yolo, clip_model, clip_preprocess, augment=False)
        train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0, drop_last=True)
        val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
        
        # 앙상블 모델 생성
        model = EnsembleImageHarmfulClassifier(yolo_dim, clip_dim, ENSEMBLE_MODELS).to(DEVICE)
        
        # 손실함수, 옵티마이저 설정
        criterion = StrongLabelSmoothingBCELoss(smoothing=0.2)
        optimizer = optim.Adam(model.parameters(), lr=IMAGE_LR, weight_decay=IMAGE_WEIGHT_DECAY)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=2)
        
        best_f1 = 0
        patience, patience_count = 2, 0
        ...
        for epoch in range(IMAGE_EPOCHS):
            ...
            for features, labels in tqdm(train_loader, desc=f"Fold {fold+1} Epoch {epoch+1}"):
            ...
            if len(val_true) > 0:
                ...
                print(f"  Epoch {epoch+1}: Loss={train_loss/len(train_loader):.4f}, F1={f1:.4f}")
        ...
        ensemble_models.append(model_state)
        print(f"  Fold {fold+1} Best F1: {best_f1:.4f}")
    
    # 5. 최종 앙상블 모델 생성
    print(f"\n✓ Cross Validation 완료")
    print(f"  평균 F1: {np.mean(fold_scores):.4f} ± {np.std(fold_scores):.4f}")
    print(f"  최고 F1: {np.max(fold_scores):.4f}")
    
    # 최고 성능 모델을 최종 모델로 선택
    best_fold_idx = np.argmax(fold_scores)
    best_model = EnsembleImageHarmfulClassifier(yolo_dim, clip_dim, ENSEMBLE_MODELS).to(DEVICE)
    best_model.load_state_dict(ensemble_models[best_fold_idx])
    
    # 최종 모델 저장
    torch.save(best_model.state_dict(), 'image_model_best.pth')
    print(f"✓ 최종 모델 저장 완료 (Fold {best_fold_idx+1}, F1: {fold_scores[best_fold_idx]:.4f})")
    ...
    with torch.no_grad():
        for features, labels in val_loader:
            ...
            preds = (outputs > 0.5).cpu().numpy()
            final_preds.extend(np.atleast_1d(preds))
            final_true.extend(np.atleast_1d(labels.numpy()))
    
    if len(final_true) > 0:
        precision, recall, f1, _ = precision_recall_fscore_support(final_true, final_preds, average='binary', zero_division=0)
        ...
        plt.ylabel('True')
        plt.title(f'Ensemble Image Model Confusion Matrix (F1={f1:.3f})')
...
# ============================================================
# [변경점] final_model8: Cross Validation 비디오 모델 학습
# ============================================================
def train_video_model_cv():
    """
    Cross Validation을 사용한 비디오 모델 앙상블 학습
    
    K-Fold Cross Validation을 통해 더 엄격한 평가와 안정적인 모델을 생성
    """
    print("\n" + "="*60)
    print("Cross Validation 비디오 모델 앙상블 학습")
    print("="*60)
    ...
    # 3. Cross Validation 설정
    kfold = KFold(n_splits=K_FOLDS, shuffle=True, random_state=42)
    all_train_data = list(zip(train_paths, train_labels))
    all_train_data = np.array(all_train_data, dtype=object)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(all_train_data)):
        ...
        fold_train_paths = [item[0] for item in fold_train_data]
        fold_train_labels = [item[1] for item in fold_train_data]
        fold_val_paths = [item[0] for item in fold_val_data]
        fold_val_labels = [item[1] for item in fold_val_data]
        
        # 데이터셋 및 데이터로더 생성
        train_dataset = HarmfulVideoDataset(fold_train_paths, fold_train_labels, yolo, slowfast_model, clip_model, clip_preprocess, slowfast_dim)
        val_dataset = HarmfulVideoDataset(fold_val_paths, fold_val_labels, yolo, slowfast_model, clip_model, clip_preprocess, slowfast_dim)
        ...
        # 클래스 불균형 처리
        num_harmful = sum(fold_train_labels)
        num_safe = len(fold_train_labels) - num_harmful
        pos_weight = torch.tensor([num_safe / num_harmful]).to(DEVICE)
        
        # 앙상블 모델 생성
        model = EnsembleVideoHarmfulClassifier(yolo_dim, clip_dim, slowfast_dim, ENSEMBLE_MODELS).to(DEVICE)
        ...
        for epoch in range(VIDEO_EPOCHS):
            ...
            for features, labels_batch in tqdm(train_loader, desc=f"Fold {fold+1} Epoch {epoch+1}"):
            ...
            print(f"  Epoch {epoch+1}: Loss={train_loss/len(train_loader):.4f}, F1={f1:.4f}, Th={best_th:.2f}")
        ...
        # 폴드 결과 저장
        fold_scores.append(best_f1)
        ensemble_models.append((model_state, best_threshold))
        print(f"  Fold {fold+1} Best F1: {best_f1:.4f}, Threshold: {best_threshold:.2f}")
    ...
    # 최고 성능 모델을 최종 모델로 선택
    best_fold_idx = np.argmax(fold_scores)
    best_model = EnsembleVideoHarmfulClassifier(yolo_dim, clip_dim, slowfast_dim, ENSEMBLE_MODELS).to(DEVICE)
    best_model.load_state_dict(ensemble_models[best_fold_idx][0])
    best_threshold = ensemble_models[best_fold_idx][1]
    ...
    print(f"✓ 최종 모델 저장 완료 (Fold {best_fold_idx+1}, F1: {fold_scores[best_fold_idx]:.4f}, Th: {best_threshold:.2f})")
    ...
    with torch.no_grad():
        for features, labels_batch in val_loader:
            ...
            if outputs_np.ndim == 0:
                final_outputs.append(float(outputs_np))
            else:
                final_outputs.extend(outputs_np.tolist())
            final_true.extend(labels_batch.cpu().numpy())

    # 최종 평가
    final_outputs_np = np.array(final_outputs)
    final_true_np = np.array(final_true)
    final_preds = (final_outputs_np > best_threshold).astype(int)
    
    if len(final_true_np) > 0:
        precision, recall, f1, _ = precision_recall_fscore_support(final_true_np, final_preds, average='binary', zero_division=0)
        ...
        # Confusion Matrix 생성
        cm = confusion_matrix(final_true_np, final_preds)
        plt.figure(figsize=(8, 6))
        ...
        plt.xlabel('Predicted')
        plt.ylabel('True')
        plt.title(f'Ensemble Video Model Confusion Matrix (F1={f1:.3f}, Th={best_threshold:.2f})')
...
if __name__ == "__main__":
    ...
    시스템 정보를 출력하고 Cross Validation 기반 앙상블 학습을 수행합니다.
    
    사용법:
        - 전체 학습: python final_model8.py
        - 이미지만: python final_model8.py train_image  
        - 비디오만: python final_model8.py train_video
    """
    # 시스템 정보 출력
    print("\n" + "="*60)
    print("무하유 유해 콘텐츠 탐지 시스템 - 앙상블 버전 8")
    print("="*60)
    ...
    print(f"K-Fold CV: {K_FOLDS}")  # Cross Validation 폴드 수
    print(f"Ensemble Models: {ENSEMBLE_MODELS}")  # 앙상블 모델 수
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if mode == 'train_image':
                # 이미지 모델만 학습
                train_image_model_cv()
            elif mode == 'train_video':
                # 비디오 모델만 학습
                train_video_model_cv()
            else:
                # 잘못된 인수인 경우 사용법 출력
                print("\n사용법:")
                print("  python final_model8.py               (전체 학습)")
                print("  python final_model8.py train_image   (이미지만)")
                print("  python final_model8.py train_video   (비디오만)")
        else:
            # 인수 없이 실행하면 전체 학습 수행
            train_image_model_cv()  # 이미지 모델 학습
            train_video_model_cv()  # 비디오 모델 학습
            ...
            print("="*60)
            print("생성된 파일:")
            print("  - image_model_best.pth")           # 최고 성능 이미지 앙상블 모델
            print("  - video_model_best.pth")            # 최고 성능 비디오 앙상블 모델

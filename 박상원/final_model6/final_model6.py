...
# ------------------------------------------------------------
# [변경점] final_model6: 데이터 중복 제거를 위한 hashlib 추가
# ------------------------------------------------------------
import hashlib
...
# ============================================================
# [변경점] final_model6: 이미지 중복 제거 함수 추가
# ============================================================
# 이유: 데이터 누수 방지 - 같은 이미지가 train/val에 중복되는 것을 방지
...
class VideoHarmfulClassifier(nn.Module):
    ...
    def __init__(self, yolo_dim, clip_dim, slowfast_dim):
        ...
        # ------------------------------------------------------------
        # [변경점] final_model6: Transformer Dropout 증가 (0.4→0.5)
        # ------------------------------------------------------------
        # 이유: 비디오 모델의 과적합 방지
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=0.5               # 드롭아웃 비율 증가 (0.4→0.5)
        )
        ...
        # ------------------------------------------------------------
        # [변경점] final_model6: 분류기 Dropout 증가
        # ------------------------------------------------------------
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm1d(128),
            nn.Dropout(0.5),                # 드롭아웃 증가 (0.4→0.5)
...
def prepare_image_data():
    ...
    # ------------------------------------------------------------
    # [변경점] final_model6: 중복 이미지 제거
    # ------------------------------------------------------------
    # 이유: 데이터 누수 방지 - 같은 이미지가 train/val에 중복되는 것 방지
...
# ============================================================
# [변경점] final_model6: Label Smoothing Loss 추가
# ============================================================
# 이유: 과적합 방지 - 모델이 너무 확신하는 것을 방지하여 일반화 성능 향상
...
def train_image_model():
    ...
    # ------------------------------------------------------------
    # [변경점] final_model6: 클래스 불균형 처리 추가
    # ------------------------------------------------------------
    # 이유: 이미지 데이터도 유해/안전 비율이 불균형하므로 가중치 적용
    ...
    # ------------------------------------------------------------
    # [변경점] final_model6: Label Smoothing Loss 사용
    # ------------------------------------------------------------
    ...
    for epoch in range(IMAGE_EPOCHS):
        ...
        for features, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{IMAGE_EPOCHS}"):
            ...
            # ------------------------------------------------------------
            # [변경점] final_model6: Gradient Clipping 추가
            # ------------------------------------------------------------
            # 이유: 그래디언트 폭발 방지, 안정적인 학습
...
def train_video_model():
    ...
    # ------------------------------------------------------------
    # [변경점] final_model6: 비디오에도 Learning Rate Scheduler 추가
    # ------------------------------------------------------------
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=2)
    ...
    for epoch in range(VIDEO_EPOCHS):
        ...
        for features, labels_batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{VIDEO_EPOCHS}"):
            ...
            # ------------------------------------------------------------
            # [변경점] final_model6: 비디오에도 Gradient Clipping 추가
            # ------------------------------------------------------------
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
...
if __name__ == "__main__":
    ...
    사용법:
        - 전체 학습: python final_model6.py
        - 이미지만: python final_model6.py train_image  
        - 비디오만: python final_model6.py train_video
    """
    # 시스템 정보 출력
    print("\n" + "="*60)
    print("무하유 유해 콘텐츠 탐지 시스템 - 개선 버전 6")
    print("="*60)
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if mode == 'train_image':
                ...
                # 잘못된 인수인 경우 사용법 출력
                print("\n사용법:")
                print("  python final_model6.py               (전체 학습)")
                print("  python final_model6.py train_image   (이미지만)")
                print("  python final_model6.py train_video   (비디오만)")
        else:
            # 인수 없이 실행하면 전체 학습 수행
